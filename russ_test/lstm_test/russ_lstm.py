...
def load_weather_frame(filename):
    ...
    data_raw['DryBulbCelsius'] = data_raw['DryBulbCelsius'].astype(float)  # DryBulbCelsius
    times = []
    ...
    df = pd.DataFrame(data_raw, columns=['_time', 'WetBulbCelsius', 'DryBulbCelsius'])
    res = df.set_index('_time')
